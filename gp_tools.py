...
class GpRegressor:
    ...
    :param x: The spatial coordinates of the y-data values. For the 1-dimensional case, \
    ...
    :param y_err: The error on the y-data values supplied as a list or array of floats. \
    ...
    :param scale_lengths: The scale-length over which the data varies in each dimension supplied \
                          as a list or array of floats. This argument is only used in the case of \
                          multi-dimensional regression. GPR sets the covariance between two points \
                          by the euclidean distance between them as calculated by the coordinates \
                          given in the 'x' argument. If the scale-length of variation in each \
                          dimension varies greatly, using an unweighted euclidiean distance will \
                          produce erroneous results. Instead, distances in each dimension are \
                          normalised via division by the values provided in `scale_lengths` before \
                          the point-to-point distance is calculated.

    :param hyperpars: The amplitude and scale-length parameters for the normal prior distribution. \
                      If not specified, the optimal parameter values are determined automatically \
                      using a global optimisation approach.
    """
    ...
    def evaluate_parallel(self, q, threads = None):
        if threads is None:
            threads = 4
